...
export function KickoffClient() {
  ...
  useEffect(() => {
    fetch("/api/availability?purpose=kickoff")
      .then((r) => r.json())
      .then((d) => {
        setSlots(d.slots || {});
  ...
  async function handleBook() {
    ...
    if (!name.trim() || !email.trim() || !company.trim()) {
      setError("Necesitamos tu nombre, email y empresa para confirmar.");
      return;
    }
    setBooking(true);
    setError("");
  ...
  const currentDateIdx = selectedDate
    ...
    <>
      ...
      <main className="relative bg-brand-black text-white min-h-screen pt-28 pb-20 px-4">
        ...
        <div className="relative max-w-2xl mx-auto">
          ...
          >
            <span className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-[10px] font-semibold text-emerald-400 tracking-[0.2em] uppercase mb-4">
          ...
          {/* Datos de contacto (siempre visibles, prefilled si vienen de checkout) */}
          <motion.div
          ...
          {loading ? (
            ...
            <div className="text-center py-16">
              ...
              <a
                href={`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent("Hola, acabo de pagar mi plan UGC y quiero agendar mi reunión de inicio.")}`}
                target="_blank"
            ...
            >
              ...
              {selectedSlot && (
                ...
                >
                  <Button
                    size="xl"
                    className="w-full gap-2 bg-emerald-500 hover:bg-emerald-600 text-black"
                    onClick={handleBook}
                    disabled={booking}
                  >
                    {booking ? (
                      ...
                      <>
                        ...
                        Confirmar a las {selectedSlot.time_label}
