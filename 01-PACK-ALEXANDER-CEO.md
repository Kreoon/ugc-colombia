# PACK OPERATIVO — ALEXANDER CAST
## Founder / CEO · UGC Colombia

**Fecha:** 2026-04-09
**Versión:** 1.0
**Rol:** Founder, Estrategia, Ventas, Renewal
**Uso:** Lectura offline en Drive. Referencia de cabecera para revisiones semanales, llamadas comerciales y renewals.

---

## Tabla de contenidos

1. [Resumen ejecutivo](#1-resumen-ejecutivo)
2. [OKRs Q2-Q3 2026](#2-okrs-q2-q3-2026)
3. [Dashboard del CEO](#3-dashboard-del-ceo)
4. [Decisiones pendientes que bloquean Q2](#4-decisiones-pendientes)
5. [Riesgos Top 5](#5-riesgos-top-5)
6. [Script de discovery call](#6-script-discovery-call)
7. [Plan de contenido 90 días (LinkedIn + multicanal)](#7-plan-90-dias)
8. [Renewal call playbook — Día 90](#8-renewal-call-playbook)
9. [Cadencia de rituales](#9-cadencia-rituales)

---

## 1. Resumen ejecutivo

**Empresa:** UGC Colombia (Infiny Group SAS)
**Fecha base:** 2026-04-08 (Mes 0 — arranque Q2 2026)
**Meta 12 meses:** $100K USD/mes de MRR activo cobrado
**Ticket promedio:** $750 USD/mes (Plans: Starter $500 / Pro $950 / Premium $1.500)
**Ruta validada:** 10 clientes M3 → 35 clientes M6 → 110 clientes M12
**Equipo:** Alexander (CEO), Brian (Finanzas), Diana (Head of Ops / Creadores), Samuel (Tech), Tanya (Comunidades)

**North Star Metric (NSM):** **MRR activo cobrado en USD**. Una sola métrica que captura salud comercial, retención y cobranza efectiva. No vale "vendido", vale "cobrado y activo".

**Árbol de métricas derivadas:**
- Nuevo MRR (new logos) ← Leads calificados × Close rate × CAC
- MRR expansion (upsells) ← Upsell rate × NPS
- MRR perdido (-) ← Logo churn + Revenue churn (downgrades)

**Counter-metrics (anti-gaming):**
- Gross Margin % (no quemar plata cerrando barato)
- Delivery Health Score (1-10) — calidad por cliente
- Creator NPS — salud del lado de oferta

**Supuestos clave:**
- CAC target: $300 USD → LTV/CAC = 18x
- Payback: 4-5 meses
- Burn aceptable Q2: ≤$8K USD
- Punto de equilibrio: ~12 clientes activos ($9K MRR)

---

## 2. OKRs Q2-Q3 2026

### Q2 2026 (Abril – Junio) — Tema: "Cimientos que escalan"

#### O1 — Activar la máquina comercial y cerrar los primeros 10 clientes recurrentes

| KR | Target Jun 30 | Owner |
|---|---|---|
| KR1.1: MRR activo cobrado | **$7.500 USD** (10 clientes × $750) | Alexander |
| KR1.2: Leads calificados (SQL)/mes | **40/mes en junio** | Alexander + Tanya |
| KR1.3: Close rate de SQL → cliente | **≥25%** | Alexander |
| KR1.4: CAC blended | **≤$300 USD** | Brian |
| KR1.5: Pipeline cubierto (3x target) | **$22.500 activo** | Alexander |

**Iniciativas clave:**
- Lanzar landing UGC Colombia con calculadora de ROI y Calendly
- Outbound manual: 30 cuentas/día en IG + 20 emails/día (Alexander)
- Caso de estudio "antes/después" con 2 marcas piloto (gratis o costo)
- Webinar mensual "Cómo escalar UGC en LATAM 2026"
- Pricing oficial publicado: Starter / Pro / Premium

**Dependencias:** KREOON web stack (Samuel), Jarvis para agendamiento WhatsApp
**Riesgos:** Pipeline pobre por falta de tráfico orgánico → mitigar con Meta Ads $1K/mes

#### O2 — Construir red de creadores confiable y delivery repetible

| KR | Target Jun 30 | Owner |
|---|---|---|
| KR2.1: Creadores activos verificados | **80 creadores** | Diana |
| KR2.2: Tiempo brief → entrega final | **≤7 días** | Diana |
| KR2.3: Delivery Health Score promedio | **≥8.5/10** | Diana |
| KR2.4: % entregables aprobados sin retrabajo | **≥80%** | Diana |
| KR2.5: Creator NPS | **≥50** | Diana |

**Iniciativas:** Casting masivo, onboarding deck + contrato estándar, base de datos con tags, playbook de briefs por vertical, QA doble.

#### O3 — Establecer base operativa, financiera y de marca

| KR | Target Jun 30 | Owner |
|---|---|---|
| KR3.1: Stack operativo en producción | **100% operativo** | Samuel + Brian |
| KR3.2: Margen bruto promedio | **≥55%** | Brian |
| KR3.3: DSO (días de cobranza) | **≤15 días** | Brian |
| KR3.4: Followers IG @agenciaugccolombia | **+5.000 netos** | Tanya |
| KR3.5: Engagement rate IG | **≥4.5%** | Tanya |

### Q3 2026 (Julio – Septiembre) — Tema: "Repetibilidad"

**O1 — Escalar a 35 clientes activos manteniendo calidad**
- MRR activo cobrado: **$26.250 USD** (35 × $750)
- Logo churn mensual: **≤3%**
- NRR: **≥105%** (upsell > churn)
- SQL/mes: **120**
- Close rate sostenido: **≥30%**

**O2 — Productizar las 3 ofertas y lanzar Premium $1.500**
- % clientes en Pro o Premium: **≥60%**
- SOPs documentados por servicio: **100%**
- Time-to-onboard nuevo cliente: **≤48h**
- Roster de creadores: **200 activos**

**O3 — Activar canal de partners y referidos**
- % MRR proveniente de referidos: **≥25%**
- Partners activos: **10**
- Casos de estudio publicados: **8**
- Apariciones de Alexander (speaking / podcasts): **6**

---

## 3. Dashboard del CEO

**Diario (5 min):**
- MRR cobrado hoy
- Llamadas comerciales agendadas hoy
- Leads nuevos calificados (SQL)
- Cash en cuenta

**Semanal (lunes 9am):**
- Pipeline por etapa (lead → demo → propuesta → cierre)
- Forecast del mes vs actual
- Top 3 cuentas en riesgo de churn
- NPS de la semana

**Mensual:**
- MRR / ARR / Growth %
- CAC, LTV, LTV/CAC, payback period
- NRR, Gross/Net churn
- P&L y burn rate

---

## 4. Decisiones pendientes

> Estas decisiones **bloquean** el arranque de Q2. Cerrarlas antes del viernes 11 abril.

1. **Pricing definitivo y método de cobro.** ¿Stripe + Wise? ¿Aceptamos COP o solo USD? ¿Descuento por pago anual? → **lunes** · Bloquea: landing, contratos, propuestas.
2. **Stack CRM: Atomic CRM (Supabase) vs HubSpot Free vs Notion.** → **martes** · Bloquea: Samuel arranca dev o conecta cuentas.
3. **Presupuesto Meta Ads Q2 ($3K trimestre).** ¿Capital propio o reinversión primera factura? → **martes** · Bloquea: lanzamiento de campañas semana 2.
4. **Los 2 pilotos gratis: ¿qué marcas?** → **miércoles** · Bloquea: primer entregable real + primer caso de estudio.
5. **Roles oficiales y comp del equipo.** ¿Brian, Diana, Samuel, Tanya son socios, equity, fijo, variable? → **jueves** · Bloquea: compromiso real del equipo.
6. **Dominio y marca: ¿UGCColombia.com? ¿UGCLatam.com?** → **lunes** · Bloquea: Samuel arranca landing.
7. **Política de uso de Jarvis comercial.** ¿Responde como "Alexander"? ¿Como bot? ¿Mix? → **miércoles** · Bloquea: integración WhatsApp comercial.

---

## 5. Riesgos Top 5

| # | Riesgo | Prob. | Impacto | Mitigación | Owner |
|---|---|---|---|---|---|
| 1 | No llegar a 10 clientes en M3 por pipeline pobre | Alta | Alto | Outbound diario obligatorio + Meta Ads $1K/mes desde semana 2 + 2 pilotos gratis | Alexander |
| 2 | Calidad inconsistente de creadores → churn temprano | Alta | Crítico | Casting riguroso + QA doble + escrow + rating + roster 3x sobre demanda | Diana |
| 3 | Cash flow negativo por pago lento USA | Media | Alto | 100% anticipado mes 1, débito Stripe, factor de crédito a $20K MRR | Brian |
| 4 | Sobrecarga del equipo (5 personas haciendo todo) | Alta | Medio | Roles claros desde día 1, no contratar hasta M4, automatizar con Jarvis + KREOON | Alexander |
| 5 | Dependencia de Alexander para cerrar ventas | Alta | Alto | Documentar playbook, grabar llamadas, SDR en M5, Jarvis califica L1 | Alexander |

---

## 6. Script discovery call

### Contexto
La discovery call es para leads que **ya pasaron BANT** (Budget ≥ $1.500 USD, Authority, Need, Timing ≤ 90 días). Duración: 30 min. Objetivo: calificar y cerrar propuesta escrita en 48h.

### Estructura (30 min)

| Min | Bloque | Objetivo |
|---|---|---|
| 0–3 | Rompehielo + micro-bio del lead | Crear rapport, confirmar rol |
| 3–8 | Contexto de negocio (producto, ticket, canal actual) | Entender ICP real |
| 8–15 | Dolor específico (qué no funciona hoy en su creative) | Detectar palanca emocional |
| 15–22 | Visión de resultado (cómo se ve el éxito a 90 días) | Anclar expectativa medible |
| 22–27 | Propuesta de siguiente paso (plan + precio tentativo) | Cerrar commitment |
| 27–30 | Next steps + agenda de envío de propuesta en 48h | Cerrar loop |

### Preguntas clave (de memoria)

1. "¿Cuánto invertiste en creative en los últimos 90 días y en qué canales?"
2. "¿Qué pieza te dio el mejor ROAS últimamente? ¿Quién la hizo?"
3. "¿Quién firma una propuesta de $1.500/mes en tu empresa?" (authority)
4. "Si en 90 días bajamos tu CPA un X%, ¿qué haces con el ahorro?" (vision)
5. "¿Qué te haría decir 'no' a trabajar con nosotros?" (objeción preventiva)

### Reglas de oro

- **No prometer precio en vivo.** La propuesta se envía por escrito en 48h.
- **No bajar precio en la call.** Si hay objeción de precio, se documenta y se responde en propuesta.
- **Siempre grabar** (con consentimiento) con Granola → sube transcripción a Notion.
- **Si el lead no califica** (budget <$1K, timing >6 meses), agradece, ofrece lead magnet PDF, no lo fuerces.
- **Escalado a Alexander solo si ticket >$1.5K** (Premium). Tickets menores los cierra el SDR/Jarvis calificado.

### Post-call (mismo día)

1. Transcripción subida a Notion en 2h.
2. Resumen BANT actualizado en Supabase `leads.bant`.
3. Draft de propuesta en 24h → enviada en 48h.
4. Seguimiento WhatsApp 72h post-envío si no responde.

---

## 7. Plan 90 días (LinkedIn + multicanal)

### Objetivo
Posicionar a Alexander como la referencia #1 de UGC hispano y usar su marca personal como canal de adquisición para UGC Colombia + KREOON.

### Resumen ejecutivo del plan

| Plataforma | Frecuencia semanal | Total 90 días |
|---|---|---|
| LinkedIn | 4 posts + 1 newsletter/mes + 1 long-form/mes | ~52 posts + 3 newsletters + 3 long-forms |
| X/Twitter | 3 posts + 1 thread | ~52 piezas |
| Instagram @infiny | 3 reels + 3 carruseles | ~78 piezas |
| TikTok | 4 videos (repurpose IG) | ~48 videos |
| YouTube | 1 video long-form/mes | 3 videos |

**Total ~237 piezas en 90 días.** Ratio de repurposing: 1 long-form → 15 micro-piezas.

### Narrativa por mes

**ABRIL — "Construyendo UGC Colombia en público"**
Build in public. Alexander muestra proceso real: números, errores, decisiones. Tensión narrativa: "Nadie está construyendo esto en LATAM y nosotros lo estamos haciendo ahora".

**MAYO — "Por qué el UGC hispano es una oportunidad de $B"**
Thought leadership macro. Mercado hispano USA (62M personas, $3.4T poder adquisitivo) + LATAM digital. Tensión: "Las agencias gringas no pueden hacer esto. Nosotros sí."

**JUNIO — "Casos reales: cómo hacemos UGC que convierte"**
Social proof con metodología. Resultados específicos con mecánica detrás. Tensión: "Esto ya funciona. La pregunta es si tu marca va a aprovechar Q3 o no."

### Headline LinkedIn optimizado

```
Fundador UGC Colombia + KREOON | Performance Creative para marcas DTC
en LATAM y USA Hispanic | Agencia · SaaS · Comunidad de creadores
```

### About LinkedIn (versión aprobada)

> Si tu marca gasta en Meta Ads pero tus videos parecen producción de televisión de los 90, tienes un problema de contenido, no de presupuesto.
>
> Yo construí UGC Colombia para resolver exactamente eso.
>
> Soy Alexander Cast, fundador de UGC Colombia (la agencia de performance creative líder en LATAM), KREOON (la plataforma tech que gestiona el flujo entre marcas y creadores) e Infiny Group (el holding que conecta ambos mundos).
>
> Lo que hago diferente:
> — Opero la agencia, por eso sé dónde falla la ejecución
> — Construí la tecnología, por eso sé dónde fallan los procesos
> — Vivo en el mercado hispano, por eso entiendo el contexto cultural que los gringos pasan por alto
>
> Si eres founder DTC o lider de marketing en una marca con presupuesto activo en Meta/TikTok, hablamos.
> Contacto directo: founder@kreoon.com

### Pilares de contenido personal (4 pilares, mix semanal)

| Pilar | % | Posts/sem | Objetivo |
|---|---|---|---|
| Educación UGC y Performance Creative | 35% | 2 | Posicionarse como experto técnico |
| Construcción de negocio en LATAM | 25% | 1-2 | Resonar con founders |
| Detrás de cámara (agencia + stack) | 25% | 1 | Humanizar, generar confianza |
| Provocación y punto de vista | 15% | 1 | Alcance orgánico, debate |

### Hooks clave del primer mes (20 posts)

1. "El 80% de las marcas en LATAM están pagando por UGC falso sin saberlo."
2. "Hace 2 años no tenía ni un cliente. Hoy opero la primera agencia UGC de Colombia."
3. "La razón por la que tus ads de Meta no convierten no es el targeting."
4. "Esto es lo que pasa en un brief de UGC que nadie te muestra."
5. "Opinión impopular: la mayoría de 'agencias UGC' en LATAM son intermediarios con Canva."
6. "5 errores que cometen las marcas DTC cuando contratan UGC por primera vez."
7. "Construir tecnología mientras operas una agencia es una locura. Lo hice de todas formas."
8. "Analicé 50 ads de marcas DTC en LATAM. El patrón que encontré es preocupante."
9. "Nuestro proceso de selección de creadores tiene 7 filtros. El que más falla te sorprendería."
10. "Si tu agencia no habla de 'hook rate' y 'thumb stop ratio', cambia de agencia."
11. "Cómo pasamos de 0 a tener creadores en 8 países de LATAM sin gastar en publicidad."
12. "El formato de UGC que mejor convierte en Meta Ads en este momento (con datos)."
13. "Pregunta directa: ¿cuánto de tu presupuesto de ads va a producción de creative?"
14. "Detrás de una entrega de UGC: lo que el cliente ve y lo que pasa por dentro."
15. "Tres cosas que aprendes sobre ventas cuando tu producto es contenido auténtico."
16. "Lo que el mercado hispano de USA está buscando en UGC y ninguna agencia gringa puede darlo."
17. "Errores que cometí en el primer año que no volveré a cometer."
18. "Esto es KREOON: la plataforma que construí porque gestionarlo en hojas me tenía loco."
19. "Cuánto cobra una agencia UGC seria en LATAM. Números reales, sin rodeos."
20. "30 días posteando en LinkedIn sobre UGC. Esto aprendí del algoritmo y del mercado."

### Calendario semanal (bloques recurrentes)

- **Lunes:** LinkedIn (post principal) + X + planificación semana
- **Martes:** Instagram Reel + TikTok (repurpose)
- **Miércoles:** LinkedIn (carrusel o long-form) + X
- **Jueves:** Instagram carrusel + TikTok + X Thread
- **Viernes:** LinkedIn (cierre semana) + IG Reel + TikTok
- **Fin de semana:** Métricas + experimento + ideación

**Tiempo estimado:** ~14.5 h/semana de producción activa. Recomendación: bloque de grabación masiva martes en la mañana (3-4 reels de una sola vez).

### Sistema de repurposing (1 long-form → 15 micro-piezas)

```
VIDEO YOUTUBE (20-40 min)
├── 4 CLIPS directos (hook, controversia, dato, cierre) → TikTok + Reel
├── TEXTO derivado: 1 LinkedIn largo + 1 X Thread + 1 Newsletter + 1 caption IG
├── VISUALES: 1 carrusel LinkedIn + 1 carrusel IG + 1 quote card
└── AUDIO/REELS: 1 Reel IG + 1 TikTok + 1 secuencia Stories
Total: 15 piezas por long-form. Tiempo: ~4.5h.
```

### Newsletter "Performance Creative LATAM"

Frecuencia semanal (martes). Extensión 600-900 palabras. Primeras 8 ediciones:
1. Por qué el UGC malo es peor que no hacer publicidad
2. Anatomía de un hook que para el scroll en 3 segundos
3. Cómo estructurar el primer brief de UGC de una marca
4. El mercado hispano de USA — por qué es diferente y cómo atacarlo
5. Métricas de performance creative que todo media buyer debería conocer
6. Cómo se construye una red de creadores confiables en LATAM
7. KREOON — por qué construí tecnología desde la operación
8. Los 5 errores que más caro cuestan a las marcas DTC en su primer trimestre

### KPIs mensuales del plan 90 días

- Seguidores nuevos: +500/mes (primeros 3 meses)
- Alcance promedio por post: 2x el número de conexiones
- Comentarios por post: mínimo 10
- DMs de prospectos calificados: 5+ por semana al mes 2
- Leads atribuidos a LinkedIn como canal de descubrimiento

---

## 8. Renewal call playbook (Día 90)

### Contexto
Día 90 es la primera gran puerta de renovación. Para ese momento ya tenemos: 3 batches entregados, 3 meses de performance data, 2 rondas de NPS, health score calculado, señales claras de riesgo o expansión.

**Objetivo:** cerrar renewal de 3 meses adicionales mínimo, idealmente 6 meses, con upgrade de plan si aplica.

### Pre-call (Día 88 — Diana + Samuel preparan)
- Samuel prepara "Quarterly Performance Report" en Looker Studio + slides.
- Diana actualiza health score del cliente.
- Alexander revisa histórico y define objetivo de la call (renew, upgrade, downgrade, churn prevention).

### Estructura

**Escenario A — Health score ≥ 70 (cliente sano, renew esperado)**

| Min | Tema |
|---|---|
| 0–10 | Recap de 90 días: qué logramos juntos |
| 10–20 | Performance: top piezas, CPA evolution, ROAS |
| 20–30 | Visión próximos 90 días: qué pillars mantener, qué agregar |
| 30–40 | Propuesta comercial: renew + posible upgrade |
| 40–45 | Firma o commitment |

**Escenario B — Health score < 70 (riesgo, churn prevention)**

Tácticas:
1. **No hablar de renewal primero.** Abrir con "¿qué ha fallado y cómo lo arreglamos?"
2. Ofrecer **1 mes de ajuste** con plan de mejora documentado (no descuento).
3. Si hay bloqueo por feedback del cliente, mostrar data: "Este mes estuvimos bloqueados 9 días esperando tu feedback. Si lo agilizamos a 2 días, podemos entregar 30% más piezas."
4. Si el cliente sigue indeciso: ofrecer **downgrade transitorio** a Starter por 1 mes.
5. Si el cliente churnea: activar campaña win-back automática (día 100, 120, 150 vía Jarvis).

### Entregables de la call
- Contrato de renovación firmado (ideal 3-6 meses).
- Plan de trabajo Q2 documentado.
- Cliente movido a `clients.stage='recurring'` en Supabase.

### KPIs
- % clientes renovados al día 90: **meta 70%**
- % upgrades al renovar: **meta 25%**
- LTV promedio al día 90: **meta USD $3.000+**

### Integración Jarvis
- Jarvis envía "Quarterly Report" previo a la call.
- Jarvis envía contrato de renovación post-call con link de firma.
- Jarvis dispara win-back si cliente declina.

---

## 9. Cadencia de rituales

| Ritual | Frecuencia | Día/Hora | Duración | Asistentes |
|---|---|---|---|---|
| Daily standup | Diario | L-V 8:30am | 15 min | Todo el equipo |
| Weekly Commercial Review | Semanal | Lunes 9am | 45 min | Alexander + Brian + Tanya |
| Weekly Delivery Review | Semanal | Martes 9am | 45 min | Diana + Samuel + Alexander |
| Bi-weekly Tech Sync | Quincenal | Jueves 4pm | 30 min | Samuel + Alexander |
| Monthly Business Review | Mensual | 1er lunes | 2h | Equipo completo |
| Quarterly Business Review | Trimestral | Última semana Q | Medio día | Equipo + asesores |
| 1:1 con cada lead | Quincenal | Variable | 30 min | Alexander + cada uno |

**Reglas:** standup máximo 15 min, parado, sin laptops. Toda reunión con agenda 24h antes + notas en Notion. MBR usa formato Wins / Losses / Learnings / Next bets.

---

## Sinergias del ecosistema (recordatorio estratégico)

- **UGC Colombia ↔ KREOON:** UGC es el cliente cero de KREOON. Samuel construye módulo "Briefs y Creadores" en KREOON usando UGC Colombia.
- **UGC Colombia ↔ Infiny Group:** Oferta combinada "UGC + Ads Management" como upsell Premium ($1.500+).
- **UGC Colombia ↔ Jarvis:** Meta 80% de SQL agendados sin intervención humana. Calificación BANT automática.
- **UGC Colombia ↔ Alexander Brand:** Autoridad cierra más rápido y a precio mayor. Casos reales alimentan contenido.

---

**Fuente:** `content/sistemas/negocio/okrs-y-metricas.md`, `content/canales/alexander-90dias-content-plan.md`, `content/canales/linkedin-alexander-autoridad.md`, `content/sistemas/onboarding/client-onboarding-flow.md` (Etapa 2 + Etapa 10 + Playbook cliente difícil).

**Próxima revisión:** 2026-05-09
